 ...
 */
export default function GalleryLayoutToggle({ value, onChange, showCurated = false, modes }) {
  const visibleModes = modes || [...(showCurated ? ['curated'] : []), 'wall', 'grid']
  if (visibleModes.length <= 1) return null

  return (
    <div className="ui-gallery-layout-toggle" role="group" aria-label="作品の表示方法">
      {visibleModes.includes('curated') && (
        <button
          type="button"
